...
void CAN_Motor_Call_Back(Struct_CAN_Rx_Buffer *Rx_Buffer)
{
  switch (Rx_Buffer->Header.StdId)
  {
    case (0x202):
    {
        Motor_Lift_Rear.CAN_RxCpltCallback(Rx_Buffer->Data);
    ...
    case (0x201):
    {
        Motor_Lift_Front.CAN_RxCpltCallback(Rx_Buffer->Data);  
        break;
    } 
  
    case (0x203):
    {
        Motor_Wheel_L.CAN_RxCpltCallback(Rx_Buffer->Data);
        break;
    }
    case (0x204):
    {
        Motor_Wheel_R.CAN_RxCpltCallback(Rx_Buffer->Data);
        break;
    }
    case (0x205):
    {
        G_Chassis_Motor[0].CAN_RxCpltCallback(Rx_Buffer->Data);
        break;
    }
    case (0x206):
    {
        G_Chassis_Motor[1].CAN_RxCpltCallback(Rx_Buffer->Data);
        break;
    }
    case (0x207):
    {
        G_Chassis_Motor[2].CAN_RxCpltCallback(Rx_Buffer->Data);
        break;
    }
    case (0x208):
    {
        G_Chassis_Motor[3].CAN_RxCpltCallback(Rx_Buffer->Data);
        break;
    }
  }
}
...
int main(void)
{
  ...
  while (1)
  {
    // motor_3508 现在由 FreeRTOS 任务驱动 (Task3508)，主循环中不再直接调用其接口
